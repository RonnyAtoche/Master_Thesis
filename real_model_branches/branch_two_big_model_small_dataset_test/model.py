...
def create_model(input_dim, load_weights=False, filename_model=''):
  ...
  x = Conv2D(32, (18, 18), strides=(4, 4), padding='same', data_format='channels_first', kernel_initializer=initializers.RandomNormal(stddev=0.02))(x)
  x = LeakyReLU(0.2)(x)
  x = BatchNormalization()(x)
  x = Dropout(0.2)(x)
  x = MaxPooling2D(pool_size=(4, 4), strides=(2, 2), padding='same')(x)

  x = Conv2D(64, (10, 10), strides=(3, 3), padding='same')(x)
  x = LeakyReLU(0.2)(x)
  x = BatchNormalization()(x)
  x = Dropout(0.2)(x)
